...
## v1.0.31 (2026-02-20)
- **修复：后台进程回收与媒体控制消失问题**
  - **强化前台服务**: 即使在暂停状态下，现在也会强制保持前台服务标识，防止 Android 12+ 系统在锁屏期间为了省电而杀死应用进程。
  - **引入 CPU 休眠锁 (WakeLock)**: 确保在播放器启动期间，CPU 不会被系统强制进入低功耗挂起状态，保证应用内按钮和系统级控制面板的实时响应。
  - **优化通知持久性**: 将暂停状态下的通知也设置为“正在进行”且不可滑动删除，确保系统播放控件（Media Control）在锁屏十几分钟后依然常驻。
